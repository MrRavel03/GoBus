...
function pintarRutas(rutas) {
  ...
  rutasDeEstaPagina.forEach(function (ruta) {
    const esFavorito = favoritosDelUsuario.hasOwnProperty(ruta.id);

    html += `
      <a href="detalle-ruta.html?id=${ruta.id}" class="route-card p-4">
        <button type="button" class="btn-favorito ${esFavorito ? "activo" : ""}" data-ruta-id="${ruta.id}" title="Marcar como favorito">
        ...
        <div class="row align-items-center">
          ...
          <div class="col-4 col-md-3 text-end d-flex flex-column align-items-end justify-content-between h-100">
            <span class="text-secondary small d-flex align-items-center gap-1 mb-2 mt-4 pt-2">
...
// Trae los favoritos del usuario logueado y vuelve a pintar las tarjetas para que se vean los corazones llenos
async function cargarFavoritosDelUsuario() {
  const idUsuario = localStorage.getItem("usuarioId");
  favoritosDelUsuario = {};
  ...
  try {
    ...
    favoritos.forEach(function (favorito) {
      favoritosDelUsuario[favorito.ruta.id] = favorito.id;
    });

    // Si las rutas ya se habian pintado, las volvemos a pintar para reflejar los corazones
    if (rutasActualmentefiltradas.length > 0) {
      pintarRutas(rutasActualmentefiltradas);
    }

    // Si estamos en detalle-ruta.html, refleja el estado en el boton "Guardar"
...
// Escucha los clics en los corazones de las tarjetas (delegado, porque se re-dibujan cada vez)
function activarFavoritos() {
  const contenedorRutas = document.getElementById("contenedor-rutas");
  if (!contenedorRutas) return;

  contenedorRutas.addEventListener("click", function (e) {
    const boton = e.target.closest(".btn-favorito");
    if (!boton) return;

    //Evita que el clic navegue hacia detalle-ruta.html (la tarjeta es un <a>)
    e.preventDefault();
    e.stopPropagation();

    alternarFavorito(Number(boton.dataset.rutaId), boton);
  });
}

// Pinta el boton "Guardar" de detalle-ruta.html segun si la ruta actual ya es favorita
function sincronizarBotonFavoritoDetalle() {
  const boton = document.getElementById("btn-favorito-detalle");
  if (!boton || !boton.dataset.rutaId) return;

  const esFavorito = favoritosDelUsuario.hasOwnProperty(Number(boton.dataset.rutaId));
  boton.classList.toggle("activo", esFavorito);
  boton.innerHTML = esFavorito
    ? '<i data-lucide="heart" style="width: 18px;"></i> Guardado'
    : '<i data-lucide="heart" style="width: 18px;"></i> Guardar';

  if (typeof lucide !== "undefined") lucide.createIcons();
}

// Escucha el clic del boton "Guardar" en detalle-ruta.html
function activarFavoritoDetalle() {
  const boton = document.getElementById("btn-favorito-detalle");
  if (!boton) return;

  boton.addEventListener("click", async function () {
    const idRuta = Number(boton.dataset.rutaId);
    if (!idRuta) return;

    await alternarFavorito(idRuta, boton);
    sincronizarBotonFavoritoDetalle();
  });
}

// Marca o desmarca una ruta como favorita segun su estado actual
async function alternarFavorito(idRuta, boton) {
  const idUsuario = localStorage.getItem("usuarioId");

  if (!idUsuario) {
    Swal.fire({
      title: "Inicia sesion",
      text: "Necesitas una cuenta para guardar rutas favoritas",
      icon: "info",
      confirmButtonColor: "#0d6efd",
    });
    return;
  ...
  const idFavoritoExistente = favoritosDelUsuario[idRuta];

  try {
    if (idFavoritoExistente) {
      //Ya era favorita: se quita
      const res = await fetch(`/favoritos/${idFavoritoExistente}`, { method: "DELETE" });
      if (!res.ok) return;

      delete favoritosDelUsuario[idRuta];
      boton.classList.remove("activo");
    } else {
      //No era favorita: se guarda
      ...
      const favoritoCreado = await res.json();
      favoritosDelUsuario[idRuta] = favoritoCreado.id;
